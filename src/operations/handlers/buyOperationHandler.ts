 ...
 */
export const processBuyOperation = (operationsOutcome: OperationsOutcome, operation: Operation): OperationsOutcome => {
  const weightedAveragePrice = calculateWeightedAveragePrice(operationsOutcome, operation);
  return {
    ...operationsOutcome,
 ...
 */
export const calculateWeightedAveragePrice = (operationsOutcome: OperationsOutcome, operation: Operation): number => {
  const totalSharesValue = operationsOutcome.shares * operationsOutcome.weightedAveragePrice;
  const operationValue = operation.quantity * operation['unit-cost'];
  const totalSharesAfterOperation = operationsOutcome.shares + operation.quantity;

  const weightedAveragePrice = (totalSharesValue + operationValue) / totalSharesAfterOperation;
  const roundedWeightedAveragePrice = roundToTwoDecimalPlaces(weightedAveragePrice);
